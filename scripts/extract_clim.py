...
def climatology_gridded_data(params, dataset, bbox=None, keep_DataArray=False):
    ...
    tindex = get_climatology_times_index(
        xr_data['time'].values,
        tclim['output'],
        tclim['input'],
        tclim['startYear'],
    ...
    for t in tindex['syear']:
        ...
        if tindex['aggregate']:
            ...
            for i, v in enumerate(ix['index']):
                if bbox:
                    xr_ds = xr_data[params['variable']].isel(
                        time=v, lat=sindex['lat'], lon=sindex['lon']
                    )
                else:
                ...
                aggr += [
                    aggregate_timeSeries(
                        xr_ds, dataset['function'], aggr_len, tclim['minFrac']
                    )
                ]
    ...
    for poly in shpObj['polys']:
        ...
        for t in tindex['syear']:
            if not t in syear:
                ...
                clim = _clim_create_miss_gridded(
                    xr_ds, params, tclim
                )
                out_tmp += [clim]
                continue
            ...
            if tindex['aggregate']:
                ...
                for i, v in enumerate(ix['index']):
                    ...
                    aggr_len = ix['length'][i]
                    c_aggr = aggregate_timeSeries(
                        xr_ds,
                        dataset['function'],
                        aggr_len,
                        tclim['minFrac']
                    )
                    c_aggr = mask_polygons_xarray_dataArray(
                        c_aggr,
                        shpObj['shp'],
                        params['shpField'],
                        poly
...
def climatology_rectangle_point_data(params, dataset, bbox):
    xr_data = get_zarr_dataset(params)
    xr_coords = get_coords_dataset(xr_data)
    sindex = get_bbox_latlon_index(xr_coords, bbox)
    if sindex is None:
    ...
    for s in select_column:
        ...
        for t in tindex['syear']:
            ...
            if tindex['aggregate']:
                ...
                for i, v in enumerate(ix['index']):
                    xr_ds = xr_data[params['variable']]
                    xr_ds = xr_ds.isel(time=v, lat=ila, lon=ilo)
