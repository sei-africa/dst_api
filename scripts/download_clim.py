...
def _clim_polygons_grid(params):
    ...
    return _wrap_clim_gridded_data(
            climatology_polygons_grid_data,
            params
        )
...
def _clim_rectangle_point(params):
    ...
    return _wrap_clim_spoints_data(
            climatology_rectangle_point_data,
            params,
            bbox=bbox
        )
...
def _clim_polygons_points(params):
    params = _check_params_Poly(params)
    return _wrap_clim_spoints_data(
            climatology_polygons_point_data,
            params
        )

def _clim_multipoints_data(params):
    return _wrap_clim_spoints_data(
            climatology_multipoints_data,
            params
        )

def _clim_geojson_data(params):
    return _wrap_clim_spoints_data(
            climatology_geojson_data,
            params
        )
...
def _clim_netcdf_info(params):
    ...
    if params['climFunction'] == 'mean':
        ...
        if type(params['precentileValue']) is list:
            ...
            f1 = params['precentileValue']
            f2 = info['name']
            info['name'] = f"{f1}th Percentile, {f2}"
    elif params['climFunction'] == 'cv':
        f1 = info['name']
        info['name'] = f"Coefficient of Variation, {f1} Climatology"
        info['units'] = '%'
    elif params['climFunction'] == 'frequency':
        f1 = info['name']
        f2 = params['frequencyOper']
        f3 = params['frequencyThres']
        info['name'] = f"Frequency of {f1} {f2} {f3}"
        info['units'] = '%'
        ...
        f1 = params['probaThres']
        f2 = info['name']
        info['name'] = f"Probability of exceeding {f1}, {f2}"
        info['units'] = '%' if params['probaUnit'] == 'perc' else ''
        info['out_varid'] = 'proba_exceeding'
    elif params['climFunction'] == 'probNoExc':
        f1 = params['probaThres']
        f2 = info['name']
        info['name'] = f"Probability of non-exceeding {f1}, {f2}"
        info['units'] = '%' if params['probaUnit'] == 'perc' else ''
