...
def get_climatology_times_index(
    times,
    out_time_res,
    in_time_res,
    start_year,
    ...
    if out_time_res == 'daily':
        out = daily_climatology_index(
            times, start_year, end_year, day_win
        )
    elif out_time_res == 'dekadal':
        out = dekadal_climatology_index(
            times, out_time_res, in_time_res,
            start_year, end_year
        )
    elif out_time_res == 'monthly':
        out = monthly_climatology_index(
            times, out_time_res, in_time_res,
            start_year, end_year
        )
    elif out_time_res == 'seasonal':
        out = seasonal_climatology_index(
            times, out_time_res, in_time_res,
            start_year, end_year, seas_len
        )
    elif out_time_res == 'annual':
        out = annual_climatology_index(
            times, out_time_res, in_time_res,
            start_year, end_year
        )
    else:
        return {
                'status': -1,
                'message': 'Unknown output time resolution.'
            }
    ...
    if len(out_index) == 0:
        ...
        msg = f'{msg} for {m1} data to compute {m2} climatology.'
        return {'status': -1, 'message': msg}

    out['index'] = out_index
    if not full_year:
        out = filter_climatology_index(
            out, out_time_res, clim_date
        )
...
def filter_climatology_index(index, out_time_res, clim_date):
    ...
    if len(index['index']) == 0:
        ...
        msg = f'{msg} temporalRes={m1}, climDate={m2}'
        return {'status': -1, 'message': msg}
...
def monthly_climatology_index(times, out_time_res, in_time_res, start_year, end_year):
    ...
    if aggregate:
        ...
        idx = get_index_dates_aggregate(
            times_clm, out_time_res, in_time_res,
            start_date, end_date
        )
        if idx['status'] == -1:
            return idx

        times_mon = [datetime.strptime(m, '%Y%m').month for m in idx['dates']]
...
def dekadal_climatology_index(times, out_time_res, in_time_res, start_year, end_year):
    ...
    if aggregate:
        ...
        times_idx = [datetime.strptime(t, '%Y%m%d') for t in idx['dates']]
...
def seasonal_climatology_index(
    times, out_time_res, in_time_res,
    start_year, end_year, seas_len
):
    period = f'{start_year}-{end_year}'
    ...
    if aggregate:
        ...
        return {
                'status': -1,
                'message': 'No aggregation possible.'
            }
